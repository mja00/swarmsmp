...
import dev.mja00.swarmsmps2.config.ServerDataMod;
...
public class SwarmsmpS2 {
    ...
    private static ServerData convertToServerData(ServerDataMod serverDataMod) {
    ...
    public SwarmsmpS2() {
        ...
        DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> {
            ...
            if (!Files.exists(Minecraft.getInstance().gameDirectory.toPath().resolve("servers.dat"))) {
                ...
                ourServers.forEach((server) -> {
                    if (server.isForced()) {
                        for (int i = 0; i < serverList.size(); i++) {
                            ServerData existingServer = serverList.get(i);
